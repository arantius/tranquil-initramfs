    ...
    for i in $(seq 0 $((${#drives[@]} - 1))); do
        if [[ ${enc_type} == "pass" ]]; then
            while [[ ${count} -lt ${max} ]]; do
                # Putting the enc_options var in double quotes _will_ cause cryptsetup to fail
                # and display an "unknown option" message.
                echo "${code}" | cryptsetup ${enc_options} luksOpen ${drives[${i}]} vault_${i} && break
            ...
            cryptsetup --key-file "${keyfile}" ${enc_options} luksOpen ${drives[${i}]} vault_${i} || \
            die "Failed to decrypt: ${drives[${i}]}"
            ...
            while [[ ${count} -lt ${max} ]]; do
                ...
                cryptsetup --key-file=- ${enc_options} luksOpen ${drives[${i}]} vault_${i} && break
